...
function partTwo(ruleHash, list) {
  ruleHash[0].ruleRegexString =
    ruleHash[8].ruleRegexString + ruleHash[11].ruleRegexString;
  return list.reduce((carry, item) => {
    let regex = new RegExp("^" + ruleHash[0].ruleRegexString + "$");
...
function getRules(rules) {
  ...
  while (assignedRule) {
    ...
    rules.forEach((rule) => {
      ...
      let strings = rule.conditionIndexes
        .map((index) => getRule(index, rule))
        .filter(Boolean);
      let altStrings = rule.altIndexes
        .map((index) => getRule(index, rule))
        .filter(Boolean);
      if (
        strings.length === rule.conditionIndexes.length &&
        altStrings.length === rule.altIndexes.length
      ) {
        rule.ruleRegexString = "(" + strings.join("");
        if (altStrings.length) {
          rule.ruleRegexString =
            rule.ruleRegexString + "|" + altStrings.join("");
        }
